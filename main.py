...
def start_bot():
        ...
        if message.text == lzt:
            ...
            if len(row) == 0:
                ...
                bot.send_message(message.chat.id,
                                 text=f'Accès obtenu avec succès par code - {lzt}',
                                 reply_markup=menu.menu_access_yes)
    ...
    def handler_call(call):
        ...
        if call.data == 'admin_list_order_payment':
            msg = bot.edit_message_text(
                chat_id=chat_id,
                message_id=message_id,
                text=f'❗️ Liste des demandes de retrait\n\n' \
                     f'{logic.admin_list_order_payment()}'
                     f'\n❗️ Entrez votre numéro de demande pour plus informations',
        ...
        if call.data == 'go_main_menu':
            ...
            msg = bot.send_message(chat_id, '❗️ Voulez-vous quitter le menu admin ?', reply_markup=markup)
    ...
    def order_payout_2(message):
        try:
            ...
            if float(message.text) < settings.MIN_PAYOUT:
                bot.send_message(message.chat.id,
                                 text='❌ Le montant saisi est inférieur au minimum',
                                 reply_markup=menu.menu_access_yes)
            if float(message.text) > balance.balance:
                bot.send_message(message.chat.id,
                                 text="❌ Il n'y a pas assez de fonds au bilanт",
                                 reply_markup=menu.menu_access_yes)
            if float(message.text) >= settings.MIN_PAYOUT and float(message.text) <= float(balance.balance):
                msg = bot.send_message(message.chat.id,
                                       text=f'❗️ solde - {balance.sum}\n'
                                            f'❗️ Entrez votre adresse usdt , le paiement lui sera effectué!\n')
    ...
    def order_payout_3(message):
        ...
        msg = bot.send_message(text=f'❗️ Vérifiez les données et confirmez la demande\n\n'
                               f'❕ Solde - {balance.sum}\n'
                               f'❕ adresse - {balance.adresse}\n\n'
                               f'❗️ Demande de retrait ?',
